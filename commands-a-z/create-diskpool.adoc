...
`resourceProvisioningCapable`
 a| 
Si ce paramètre est défini sur `TRUE`, seuls les disques compatibles DULBE seront utilisés pour créer des pools de disques ou des groupes de volumes. Si le paramètre est défini sur `FALSE` ou n'est pas fourni, aucune vérification de la compatibilité DULBE n'est effectuée. Si tous les disques sélectionnés pour le groupe de volumes ou le pool de disques sont compatibles DULBE, alors l'indicateur au niveau de la baie de stockage détermine si le groupe de volumes ou le pool de disques est créé comme étant à ressources provisionnées.
